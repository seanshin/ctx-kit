...
(할인율을 → 할인율도 매칭). 영어 토큰은 가벼운 접미사 정규화를 거친다 —
복수형(`secrets` → `secret`), 동명사(`scanning` → `scan`), 분사(`detected` →
`detect`) — 그래서 "secret scanning"이 코드의 `Scans`/`secrets` 식별자에
정확한 활용형을 몰라도 닿는다. 사전 기반이 아니라 보수적인 접미사 제거라서
한계가 있다: 4자 미만 토큰은 건드리지 않고, 무음 e로 끝나는 동사는 완전히
합쳐지지 않는다(`generated`/`generate`, `included`/`include`는 각각
`generat`/`includ`로 축약돼 서로 매칭되지 않는다 — 틀린 병합보다 놓치는 쪽을
택한 것). `-tion`/`-sion` 명사화는 아예 접지 않는다: `session` → `sess`,
`nation` → `nat`처럼 실재하는 다른 단어와 충돌하는 경우가 도움이 되는
경우보다 훨씬 많기 때문이다.
...
토큰 수는 휴리스틱이다: ASCII는 `문자수/4`, CJK 코드포인트는 **실측한 0.95
토큰/글자**로 센다(한글·일문·중문은 글자당 1토큰에 가깝기 때문). 단순 `문자수/4`는
한국어 주석 파일을 28% 과소평가했고, 이 방식이 오차를 약 9%로 줄인다. ASCII 계수는
그대로다.

정확한 계수가 필요하면 선택적 토크나이저를 설치하고 명시적으로 켠다:

```sh
npm i -D gpt-tokenizer        # MIT; BPE 테이블이 ~29MB라 기본 설치에는 넣지 않음
CTXKIT_EXACT_TOKENS=1 ctxkit pack --profile light --module risk
```

"설치돼 있으면 자동"이 아니라 옵트인인 이유는, 기본값에서 모두의 계수가 같아야
공개된 측정이 재현되기 때문이다. 차이는 실재한다 — 어떤 한국어 주석 모듈에서
`light` 팩이 휴리스틱으로는 11개 파일, 정확 계수로는 9개를 담는다. 그 2개가
모델의 실제 한도를 넘겼을 것이다.
...
- [참고 자료집](docs/references.md) — 논문·도구·라이선스와 각각이 근거를 주는 결정
